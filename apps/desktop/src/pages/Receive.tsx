...
const CHAINS = [
  { id: 'bitcoin-mainnet', name: 'Bitcoin', symbol: 'BTC', color: 'bg-orange-500', textColor: 'text-orange-400', family: 'bitcoin' },
  { id: 'ethereum-1', name: 'Ethereum', symbol: 'ETH', color: 'bg-blue-500', textColor: 'text-blue-400', family: 'evm' },
  { id: 'solana-mainnet', name: 'Solana', symbol: 'SOL', color: 'bg-purple-500', textColor: 'text-purple-400', family: 'solana' },
  { id: 'polygon-137', name: 'Polygon', symbol: 'MATIC', color: 'bg-violet-500', textColor: 'text-violet-400', family: 'evm' },
  { id: 'arbitrum-42161', name: 'Arbitrum', symbol: 'ETH', color: 'bg-sky-500', textColor: 'text-sky-400', family: 'evm' },
  { id: 'base-8453', name: 'Base', symbol: 'ETH', color: 'bg-blue-600', textColor: 'text-blue-300', family: 'evm' },
  { id: 'optimism-10', name: 'Optimism', symbol: 'ETH', color: 'bg-red-500', textColor: 'text-red-400', family: 'evm' },
];
...
function QRPlaceholder({ value }: { value: string }) {
  // Generate a deterministic pattern from the address string
  const cells = 21;
  const size = 200;
    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8">
        ...
        <div className="glass-card p-8 text-center space-y-6">
          ...
          <div>
            <p className="text-xs text-zinc-500 mb-2">Your {selectedChain.name} Address</p>
            <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
              <p className="text-sm font-mono text-zinc-200 break-all leading-relaxed">{currentAddress}</p>
            </div>
          </div>
          ...
          {isBitcoin && (
            <div className="pt-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium border border-zinc-700 text-zinc-300 hover:border-zinc-500 hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {isBitcoin && (
          <div className="mt-4 flex items-start gap-3 p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-400">Avoid Address Reuse</p>
        ...
        {isBitcoin && addressIndex > 0 && (
          <div className="mt-6">
            <h3 className="text-sm font-medium text-zinc-400 mb-3">Recent Addresses</h3>
            <div className="space-y-2">
              {addresses.slice(0, addressIndex + 1).reverse().map((addr, i) => (
                <div
                  key={addr}
                  className={`glass-card p-3 flex items-center justify-between ${i === 0 ? 'border-purple-500/20' : ''}`}
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-mono text-zinc-400 truncate">{addr}</span>
                  </div>
                  <button
                    onClick={() => {
